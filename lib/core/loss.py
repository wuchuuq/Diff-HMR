...
class TCMRLoss(nn.Module):
    ...
    def forward(
        ...
        # 生成器损失
        # keypoint loss
        loss_kp_2d = self.keypoint_loss(pred_j2d, real_2d, openpose_weight=1., gt_weight=1.) * self.e_loss_weight # loss_2d
        loss_kp_3d = self.keypoint_3d_loss(pred_j3d, real_3d)
        loss_kp_3d = loss_kp_3d * self.e_3d_loss_weight # loss_3d
        ...
        # smpl loss
        real_shape, pred_shape = data_3d_theta[:, 75:], pred_theta[:, 75:] # 取shape参数和pose参数
        real_pose, pred_pose = data_3d_theta[:, 3:75], pred_theta[:, 3:75]

        loss_dict = {
            'loss_kp_2d': loss_kp_2d, # Loss_2d
            'loss_kp_3d': loss_kp_3d, # Loss_3d
        }
        if pred_theta.shape[0] > 0:
            loss_pose, loss_shape = self.smpl_losses(pred_pose, pred_shape, real_pose, real_shape) # loss_smpl
        ...
        gen_loss = torch.stack(list(loss_dict.values())).sum() # 对字典中的损失函数值进行累加求和
